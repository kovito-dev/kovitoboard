...
// User-extension structured logger (DEC-017 v1.3 §11). The `app.`
// namespace prefix is added by KB; we pass only the recipe id here.
// `as any` matches the canonical shape documented in
// docs/agent-ref/08-logging.md §3.1 so this sample reads identically
// to the recipe-author guidance.
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!startResult.success) {
      ...
      log.error(
        { err: startResult.error, jobId, theme: theme.trim(), windowName },
        'Failed to start research job window',
      )
      res.json({ jobId })
      return
    }
    ...
    if (!ready) {
      log.warn({ windowName }, 'Prompt readiness timeout, sending anyway')
    }
    ...
    // Theme is recorded in full — server-side formatters.log already
    // handles home-path masking and other PII concerns; we no longer
    // truncate here (DEC-017 audit P6-2 D-5).
    log.info({ jobId, theme: theme.trim() }, 'Job started')
    res.json({ jobId })
  } catch (err) {
    log.error({ err }, 'start-research error')
