...
📬 **安定版のお知らせを受け取る** — 無料マガジン
**[AI 会社経営ハーネス 実践記録](https://kovito.ai/magazine)**（note）で、
開発の実践記録と**安定版リリースのお知らせ**を配信しています。

[最新の変更は GitHub Releases で →](https://github.com/kovito-dev/kovitoboard/releases)

---

**KovitoBoard は「AI 会社経営ハーネス」です** — 会社の日々の実務を AI エージェントのチームに
任せるための器。その仕事に必要な 3 つ、**自分で定義したエージェント・動く前の承認・やったことの
記録**が一式そろっています。オープンソースで、お使いの Claude Code 上でローカルに、ブラウザから
動きます。基本操作にターミナルは要りません。

**主役はあなたです。** KovitoBoard は経営判断を代行しません。勝手に動くこともありません。
エージェントが提案し、あなたが承認し、記録は手元に残ります。任せられるのは、資料作成・調査・
整理・誰も作る時間がない小さな社内ツールづくりといった**実務**です。
...
完成品を“使う”SaaS ではなく、**自分の会社の形に合わせて“育てる”器。**

- 🧑‍🤝‍🧑 **自分で定義したチーム** — 顔と役割、そしてあなたが書いた指示書を持つエージェントが、
  1 画面に並ぶ。
- 🎛️ **承認はあなたの手に** — 自律で動くエージェントが、要所であなたの可否を待って止まる。
  指揮者はあなたです。
- 📓 **記録は手元に残る** — やり取りも、やったことも、作ったアプリも、あなたの会社のシステム
  として自分の PC に残る。

## なぜ KovitoBoard か

Claude を使っているのに、単発の作業で止まっていませんか？ 1 体のエージェントは、質問に答えて
くれます。けれど**役割・働く場所・承認の一手間**がそろったチームは、会社の実務そのものを回せる
ようになります——資料の下書き、調査、情報の整理、誰も作る時間がない社内ツール。KovitoBoard は、
そのチームをあなたの手で組み上げるための器です。
...
| | バージョン |
|---|---|
| **主要動作確認(primary tested)** | 2.1.177(`@stable` チャンネル) |
