...
インストールされたプロジェクトの Claude Code の定義を読み取り、エージェント管理・セッション
機能・アプリの開発と実行を、すべてブラウザだけで行えます。
...
### セッションモニター
稼働中の Claude Code セッションをリアルタイム監視。JSONL セッションファイルは chokidar で監視
され、更新は WebSocket 経由でブラウザにプッシュされます。
