...
### エージェントダッシュボード
エージェント定義の閲覧と確認。エージェントの詳細・メタデータ・元の Markdown 定義ファイルを表示できます。(v0.1.0 では読み取り専用)
...
- **設定は読み取り専用。** 設定モーダルは構成内容を表示しますが、変更を保存することはできません。
- **dist/ ESM の課題。** tsc 出力は `tsx` ランタイムを必要とします。esbuild ベースのバンドラ導入を v0.2.0 で予定しています。
