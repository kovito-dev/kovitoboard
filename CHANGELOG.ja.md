...
- browserslist の下限を `^4.28.7` に引き上げ、高危険度の警告 2 件を解消しました。
  browserslist は呼び出しのたびに、プロジェクト直下からファイルシステムのルートまでを
  遡って `browserslist-stats.json` を自動的に探します。細工されたファイルが 1 つでも
  置かれていると、プロセスが異常終了するか、オブジェクトのプロトタイプが書き換わります。
  あわせて、クエリのキャッシュが破棄されずに増え続け、最終的にメモリを使い切る問題も
  修正しました。該当のコピーは `@babel/helper-compilation-targets` 経由で間接的に
  取り込まれる開発時のみの依存で、影響が及ぶのはビルドと CI であり、利用者へは
  配布されません。下限は `overrides` で指定しているため、ロックファイルを再生成しても
  維持されます。キャレット指定なので固定ではなく下限として働きます。

- 中危険度の qs の警告 2 件も同時に解消し、6.16.0 に更新しました。角括弧キーの
  カンマ区切りによる配列上限の回避と、`isBuffer` を攻撃者が制御できることによる
  サービス妨害の 2 件です。6.16.0 は express と body-parser が宣言する範囲を
  もともと満たすため、新たなバージョン制約は追加していません。`npm audit` の検出は
  0 件になりました。
