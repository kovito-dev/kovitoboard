...
- 未対応だった高危険度の Dependabot 警告をすべて解消しました。依存パッケージの
  下限バージョンを修正版まで引き上げたもので、メジャーバージョンの更新や
  ソースコードの変更は不要でした。
  - js-yaml 4.2.0 → 4.3.1（フロントマター・エージェント YAML の解析。
    マージキーによるサービス妨害）。
  - react-router 7.17.0 → 7.18.2（ルーティング。RSC モードでの CSRF バイパスと、
    経路照合の非効率による無認証でのサービス妨害）。
  - undici → 7.29.0 および postcss → 8.5.26（いずれも開発時のみ。前者はキャッシュ
    汚染によるユーザー間の情報漏えい、後者は sourceMappingURL のパストラバーサル）、
    shell-quote → 1.10.0（開発時のみ。`parse()` の二次計算量）。いずれも利用者へは
    配布されません。

  0.2.13 で既知の問題としていた点は解消しました。gray-matter が間接的に取り込む
  js-yaml 3.14.2 は、入れ子の `overrides` で 3.15.1 に固定しています。この形なら
  gray-matter が使えない 4.x の API を強制することなく、該当コピーだけを修正版に
  できます。

- 既存のバージョン指定範囲内で解消できた中・低危険度の警告も同時に処理しました
  （nanoid 3.3.18 / dompurify 3.4.13 / body-parser 2.3.0）。`npm audit` の検出は
  0 件になりました。

- CI が、修正版の存在する高危険度の警告を検出するとビルドを失敗させるように
  なりました。セキュリティタブを見に行かなくても、作業中のブランチで気づけます。
  修正版が存在しない警告は、対処のしようがないためブロックせず報告のみ行います。

### 修正

- オンボーディング: コンシェルジュ（「コビー」）を有効にしてウィザードを完了した
  際、セッション画面が表示されるまで最大 1 分ほどフリーズする問題を修正しました。
  新規セッションのリクエストは受理された時点ですぐに応答を返し、エージェントの
  起動と初回メッセージの送信は背景で実行するようになりました（従来はエージェント
  の準備完了まで応答をブロックしていました）。
- オンボーディング: 最初のコンシェルジュセッションが、コビーが初回応答を準備する
  間に無反応に見える問題を解消しました。タイピングインジケータはセッションが開いて
  から初回応答が届くまで継続して表示され、エージェントの起動中はローディング画面
  （タイムアウト時のフォールバック付き）を表示します。
...
- セキュリティ推奨設定トースト: 「エージェントに対応を依頼」ボタンを追加しました。
  クリックすると、Claude Code の deny pattern に `.kovitoboard/` を追加するよう
  エージェントに依頼するプロンプトをあらかじめ入力した状態でセッションを起動します。
  KovitoBoard は Claude Code の設定を読み取るだけで自身で書き換えることはできないため、
  この修正作業はエージェントに委ねます。ボタンは deny pattern の推奨設定が満たされて
  いない場合にのみ表示されます。あわせて、役に立たない汎用的な「詳細を見る」リンクを
  削除しました。

### セキュリティ

- trust-prompt のフォールバックモーダルおよびデグレードモーダルが表示
  （およびコピー）する `tmux attach` スニペットを堅牢化しました。tmux の
  ウィンドウ名を二重引用符で囲んだ文字列に展開する代わりに POSIX の単一
  引用符でエンコードするようにし、シェルに貼り付けても無害なまま保たれる
  ようにしました。これは多層防御です。ウィンドウ名はサーバー側で既に
  `^[a-zA-Z0-9_-]{1,64}$` に制約されているため、シェルのメタ文字は現状
  発生しません。両モーダルが単一のヘルパーを共有するようになり、表示
  されるコマンドとクリップボードの内容が食い違うこともなくなりました。

### 修正

- 異なるエージェントによるセッション origin 予約が予約 TTL の窓内で複数
  同時に並走したときに、エージェントの取り違えが起こり得た潜在バグを修正
  しました。新規セッションがキュー先頭の予約をエージェント ID で照合せず
  に取得していたため、誤ったエージェントに紐付き、その誤マッピングが
  再起動を越えて残存する経路が成立していました。先行（eager）取得は予約が
  ちょうど 1 件のときのみ発火するようになり、キューが曖昧な場合は解決を
  agent-setting 経路（エージェント ID で照合）に委ねます。
- ライトモードで severity ステータスの色（診断バッジ・サーバーステータス
  バナー・レシピエクスポートのステータス・バージョンステータス）が薄すぎて
  読みづらかった問題を修正しました。テーマに応じた severity トークンを使う
  ようにし、各色がライト／ダーク両方の表面で読みやすくなりました。
- KB 自身の tmux セッションが初めて起動される前に、外部（KB 管理外の
  tmux）セッションがアクティブなだけで管理ステータスが `degraded` と
  誤報告される問題を修正しました。この KB プロセスが自身の tmux セッ
  ションを一度も起動していない間は、セッションが存在しないこと自体が
  KB 管理セッションの異常とはなり得ないため、ターミナルから直接起動さ
  れた外部 Claude プロセスが稼働していても degraded バナーは表示されな
  くなります。KB セッションが一度でも稼働した後は、「KB セッション稼働
  中に tmux が消失した」という真の異常は引き続き degraded として報告さ
  れます。
- 手動配置の自作アプリを並び替えられるようになりました。手動で追加した自作アプリ
  （`app/menu.ts` のエントリと `app/<id>/pages/*.tsx` だけで `manifest.json` が
  ない構成）は、manifest 不在のため並び替え対象外となり、アプリ画面のドラッグ＆
  ドロップ並び替えから外れていました。メニュースキャン時に、ページ実体が読み取り
  可能で、かつレシピインストール履歴が一切ない純粋な自作アプリに限って manifest を
  補完生成するようにし、同一スキャン内で並び替え対象に昇格させます。既存の
  manifest は決して上書きせず、壊れた manifest はそのまま残し、レシピインストール
  由来のアプリは元の出自分類を保持します。
...
- エージェントのテーマカラー: エージェント編集画面で保存した `themeColor` が、
  タイトルバー・ダッシュボード・セッション画面のアバターフレーム色に反映される
  ようになりました。以前は保存した色が既定色に戻ってしまっていました。読み込み側が
  ダブルクォート付きの hex 値しか受け付けない一方、書き込み側は（YAML として安全な
  形式である）`#` 始まりの値をシングルクォートで出力していたためです。読み込み側が
  シングルクォート・ダブルクォート・クォートなしの hex すべてを受け付けるように
  しました。
- バージョンアップ依頼セッション: バージョンアップ依頼（ステータスポップアップ）
  から起動したセッションが、「Default」ではなく正しいエージェント名で表示される
  ようになりました。バージョンアップ経路は、エージェント宛にセッションを起動する
  経路の中で唯一、起動前に origin を予約していなかったため、`/clear` から生まれた
  新しいセッションが引き継ぐべきエージェント紐付けを持たず、既定のエージェント名に
  フォールバックしていました。
...
- ライトモードで、まだ色が固定だった一部の箇所（「degraded（劣化）」ステータス
  バナーなど）の警告テキストが読みやすくなりました。以前はこれらの警告・注意系
  メッセージが固定色を使っており、ライトモードの明るい背景では文字がかすれて
  見えづらい状態でした。今後はライト・ダーク両方のテーマに追従する警告用カラー
  トークンを使用します。
- 起動時に「degraded（劣化）」ステータスバナーが誤表示される問題を修正しました。
  起動時にセッションを復元する際、ライブイベントが届く前に一瞬セッションが劣化
  状態と判定されることがありました。今後は復元されたセッションは、実際のライブ
  イベントを受信するまで待機（idle）状態を維持します（ファイル単位の復元ラッチ）。
- アイドル状態のセッションに送ったメッセージが、同じセッション内に留まり応答が
  表示されるようになりました。以前は入力が新しいセッションの開始として誤って
  扱われ、応答がいつまでも返ってこないように見えることがありました。
...
- 信頼性確認・未知プロンプトのダイアログで、実メッセージを折りたたみ
  ...
  読みやすくなっています。
- タブ式の複数質問・複数選択フォーム（Claude Code が一部の対話プロンプト
  で表示する形式）でセッションが無言のまま停止しなくなりました。
  KovitoBoard がこのフォームを検知し、「ターミナルでの操作が必要である」
  ことを案内する専用の通知（コピー可能な `tmux attach` コマンド付き）と、
  キャンセル（Esc）ボタンを表示します。このフォームは現状 KovitoBoard
  から操作できないため、フォーム固有のキーは UI に表示しません。サーバー
  側でも、これらのプロンプトに対するキャンセル以外の応答を拒否します。
...
- 推移的依存の `vite` の最低バージョンを（`package.json` の `overrides`
  ...
  インストールでも修正が維持されつつ、8.x 系の前方マイナー・パッチ更新は
  引き続き受け取れます。いずれも Windows 限定かつ開発サーバーに関する脆弱性で、
  本番ビルドには影響しません。
