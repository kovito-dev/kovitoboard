...
KovitoBoard では、不要になったアプリ（独自アプリ・レシピ由来アプリ問わず）をブラウザから削除できます。
...
- レシピサンプル画面で該当レシピは引き続き「インストール済」として表示されます
- 同じレシピを再度使いたい場合は、レシピ画面の「**再インストール**」ボタンから可能です（[`04-recipes.md`](./04-recipes.md) §3.3.1 参照）

「1 度でもインストールしたレシピはインストール済」として履歴ベースで判定する仕様のため、誤って削除した場合も再インストールで復活できます。
...
- レシピの再インストール → [`04-recipes.md`](./04-recipes.md) §3.3.1
