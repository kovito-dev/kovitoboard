...
- ✅ **削除前に対象を読み上げて再確認する。** `app/<appId>/` と `app/data/<appId>/` の中身（特に業務データ）をユーザーに提示し、消えるものを具体的に伝えてから進める（§10.2 Step 1）。
- ✅ **`app/data/<appId>/` の削除前にバックアップを提案する。** 業務データが含まれていれば、必要に応じて退避を案内する（§10.2 Step 2）。
