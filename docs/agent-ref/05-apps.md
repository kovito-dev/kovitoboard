...
- `recipe-history.jsonl`（レシピのインストール履歴。アプリ削除では更新しません — §10.5 参照）
...
- **レシピのインストール履歴は更新されません**（`recipe-history.jsonl` はそのまま）
...
- ✅ **同梱サンプル由来アプリは Disable パスを使う。** 非破壊の「無効化」（Sample apps タブ）が使えるのは **同梱サンプル（`bundled`）／grandfather サンプル**由来のアプリだけです。これらは破壊的な remove ではなく Disable に誘導すると、再有効化でデータを引き継げます（§10.0 / §10.5、[`04-recipes.md`](./04-recipes.md) §5.3）。それ以外のアプリ（独自アプリ、および v0.3.0 で再開予定の `import` / `url` 由来のレシピインストール）は Sample apps タブの無効化対象ではないため、§10.2 の削除フロー（確認 → `request-removal`）で扱います。「レシピ由来＝すべて Disable」と一般化しないこと。
...
- ❌ **`recipe-history.jsonl` をアプリ削除で書き換えない。** インストール履歴はアプリ削除では更新しない（§10.3 / §10.5）。
