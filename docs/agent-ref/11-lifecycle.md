...
## §8 プロセス衛生チェックリスト（起動前 / 停止後）

> 📖 **この節を読むタイミング:** 「再起動したのに KovitoBoard の様子がおかしい」「ポートが取られていると言われる」「前回停止したはずなのに残骸がある」とユーザーが報告したとき、または起動・停止の前後で取りこぼしがないかを能動的に検証したいとき。

KovitoBoard の起動・停止は §1〜§7 の公式コマンド（`npm start -- --project-root ..` / `npm run kb:stop`）に収束させるのが大原則です。本節は、その公式コマンドの **前後** でエージェントが行う検証手順を定義します。**`Ctrl+C` や SIGTERM で完全に掃除されたと仮定せず、必ず再検証してください。**

### §8.0 大前提 — 検証であって掃除ではない

本節のチェックは **観察と報告** が目的です。残骸を見つけても、エージェントが反射的に `pkill` / `kill -9` / `tmux kill-server` で掃除してはいけません（§3「停止時に絶対にしないこと」が引き続き優先）。残骸の **掃除は公式コマンドに委ねます**:

- 残存プロセスが見つかった → `npm run kb:stop`（必要なら `-- --force`）を実行し、終了コードに従う（§3）。
- それでも残る残骸（別プロジェクトの KB / 無関係な tsx / 手動 tmux セッション）→ **触らない**。それは KovitoBoard の管理対象外です。「KovitoBoard 由来かどうか」を判定してから、由来が確定したものだけを公式コマンドで処理します。

### §8.1 「このインスタンス由来か」の判定（最重要）

残骸を見つけたら、まず **このプロジェクトの KB 由来か** を判定してください。判定アンカー（信頼度の高い順）:

1. **PID ファイル** `<project>/kovitoboard/.kovitoboard/run/supervisor.pid` に記録された `projectRoot` / `ports`（`backend` / `vite`）/ `tmux.sessionName`。これがこのインスタンスを指す唯一の正本です。
2. **tmux セッション名** が PID ファイルの `tmux.sessionName`（通常 `kovitoboard-<プロジェクト名>`）と一致するか。
3. **ポート番号** が PID ファイルの `ports.backend` / `ports.vite` と一致するか。

**PID ファイルと結びつかないプロセス・ポート・tmux セッションは、別プロジェクトの KB か無関係なプロセスとみなし、停止対象にしません。** ホスト全体での無差別な掃除（host-wide sweep）は KovitoBoard の設計方針として行いません。

### §8.2 起動前 pre-flight

`npm start -- --project-root ..` を実行する **前** に、PID ファイル（§2 手順 3）に加えて以下を確認します:

```bash
# 1. ポート占有の確認（PID ファイルの ports、既定は backend=3001 / vite=5173）
ss -ltnp | grep -E ':(3001|5173|5174)\b'

# 2. KB 由来プロセスの確認
ps -eo pid,ppid,cmd | grep -E 'kb-start\.mjs|tsx watch src/server|node_modules/\.bin/vite|esbuild' | grep -v grep

# 3. 同名 tmux セッションの確認
tmux ls 2>/dev/null | grep '^kovitoboard-'
```

判定と対応:

- **何も出ない** → クリーン。§2 手順 4 でそのまま起動。
- **このプロジェクトの KB（§8.1 で由来確定）が既に動いている** → 二重起動しない。§2 手順 3 のとおり既存 URL を案内するか、停止が必要なら §3。
- **別プロジェクトの KB / 無関係なプロセスがポートを占有** → そのプロセスには **触らない**。`npm start` のポート auto-probe（backend 3001→3010 / vite 5173→5182）が空きを掴むため、多くの場合そのまま起動して問題ありません。起動バナーの実際のポートをユーザーに案内します。

> ℹ️ `npm start` は target ポートが埋まっていても **自動で次の空きポートに進みます**（silent fallback）。「占有されているから起動を拒否してほしい」場合は、占有元を §8.1 で特定してユーザーに報告し、ユーザーの指示を仰いでください。

### §8.3 停止後 post-stop

`npm run kb:stop` の終了コード（§3）を確認した **後**、以下を再検証します。停止前に控えておいた PID ファイルの `ports` / `tmux.sessionName` を anchor に使います（PID ファイルは正常停止で削除されるため、**停止前に値を控えておく** のがコツ）:

```bash
# 1. KB 由来プロセスの残存（停止前に控えた ports / session 名を使い、このインスタンス由来か判定）
ps -eo pid,ppid,stat,cmd | grep -E 'tsx watch src/server|node_modules/\.bin/vite|claude' | grep -v grep

# 2. ポートが解放されたか（停止前に控えた backend / vite ポート）
ss -ltnp | grep -E ':(<backend>|<vite>)\b'

# 3. tmux セッションが消えたか（停止前に控えた session 名）
tmux ls 2>/dev/null | grep '<session-name>'

# 4. ゾンビ（defunct）が残っていないか
ps -eo pid,ppid,stat,cmd | grep -E '<defunct>|Z' | grep -v grep
```
...
| 観察 | 対応 |
|---|---|
| 残存プロセスなし / ポート解放 / tmux 消滅 / ゾンビなし | クリーン。停止完了をユーザーに報告。 |
| このインスタンス由来の残存プロセスあり | `npm run kb:stop` の終了コード 4（部分成功）として既に報告されているはず。`-- --force` への昇格可否をユーザーに確認（§3）。 |
| `<defunct>`（ゾンビ）が残る | ゾンビは **親プロセス（PPID）が `wait()` していない** ことを示す。PPID が KB スーパーバイザ（既に死亡）なら、ゾンビは init（PID 1）に引き取られて自然消滅します。**`kill -9` でゾンビは消せません**（既に死んでいるため）。PPID が生存していてゾンビが滞留し続ける場合のみ、その親が誰かを §8.1 で特定してユーザーに報告。 |
| 別プロジェクトの KB / 無関係プロセスが残る | **触らない**（§8.1）。これは正常です。 |

> ℹ️ **ゾンビ（`<defunct>`）について:** ゾンビは「終了済みだが親がまだ終了ステータスを回収していないプロセス」で、CPU もメモリもほぼ消費しません。親が `wait()` するか、親が死んで init が引き取れば消えます。停止直後に一瞬見えるのは正常で、数秒後に再確認して消えていれば問題ありません。残り続ける場合だけ §8.1 で親を特定してください。

### §8.4 「Ctrl+C で止めた」場合の追加注意

ユーザーが公式コマンドではなく `Ctrl+C`（SIGINT）でスーパーバイザを止めた場合、PID ファイルは削除されますが、tmux セッションはスーパーバイザの shutdown が **触れない設計**（§8.3 tmux は tmux server に委譲）のため残ることがあります。この場合 §8.3 の再検証で tmux セッションの残存を確認し、このインスタンス由来（§8.1）と確定したら `npm run kb:stop`（PID ファイル不在時は控えておいた session 名を持つ `kb:stop`、または再起動 → 停止）で掃除します。`tmux kill-server` は使いません（§3）。
