...
describe('loadTrustPatterns 正常系', () => {
  ...
  it('DEC-015: primaryTestedVersion / bestEffortVersions を読む', () => {
    const fs = new DirectFsLayer()
    const config = loadTrustPatterns(fs, PATTERNS_JSON)
    // Tracks Anthropic's @stable dist-tag. Bumped to 2.1.177 on
    // 2026-06-15 following the @stable tag; the maintainer manually
    // verified that 2.1.177 keeps the same trust-prompt behavior as
    // 2.1.153.
    expect(config.primaryTestedVersion).toBe('2.1.177')
