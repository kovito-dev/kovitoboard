/**
 * オンボーディング §4.11 到達確認 — Phase RR-3
 *
 * オンボーディングシナリオ §4.11「独自アプリ（Intel 風機能）の実装」の
 * 到達を E2E で保証する統合テスト。
 *
 * §4.11 の検証ポイント:
 * - app/research-reports/ ディレクトリ生成（対話はモック、コピーで代替）
 * - page.tsx と 3 つの API ファイル生成
 * - ダッシュボード左サイドメニューに「Research Reports」追加
 * - テーマ入力 → レポート生成 → 表示の完遂（WebFetch はモック）
 *
 * テスト方式:
 * - app.example/research-reports/ を app/research-reports/ にコピーして
 *   「開発者エージェントがファイルを生成した」状態を再現する
 * - .kovitoboard/research-reports/ にモックデータを配置して
 *   レポート表示フローを検証する
 * - サーバーは Playwright webServer で自動起動されるため、
 *   app/ へのコピーとサーバー起動のタイミングを考慮し、
 *   API レベルでの検証を中心に行う
 *
...
test.describe('オンボーディング §4.11 到達確認 (RR-3)', () => {
  ...
  // ─── §4.11 検証: start-research API のバリデーション ─── //
  ...
  test('§4.11-V6: ファイル生成 → レポート配置 → API 取得の全フローが成立する', async ({ request }) => {
    // Step 1: app/ コピー（開発者エージェントがファイルを生成した状態を再現）
    setupAppCopy()
    expect(existsSync(APP_DIR)).toBe(true)

    // Step 2: モックレポートを配置（調査完了後の状態を再現）
    setupMockReport()

    try {
      // Step 3: list-reports で一覧取得
