 ...
 *
 * The store is persisted as a single JSON file at app/data/{appId}/_kv.json.
 * Each installed app instance keeps its own KV file, so two apps that
 * share a recipeId stay isolated. In v0.1.0, assuming low frequency
 * access, the file is read/written on each operation (no in-memory cache).
 *
