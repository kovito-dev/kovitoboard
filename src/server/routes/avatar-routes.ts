...
const avatarLog = lazyChildLogger('avatar-routes')
...
export function createAvatarRouter(fs: FileAccessLayer): Router {
  ...
  router.post(
    ...
    (req, res) => {
      ...
      try {
        ...
        avatarLog.error({ err, agentName }, 'Avatar upload failed')
  ...
  router.delete('/:name/avatar', (req, res) => {
    ...
    try {
      ...
      avatarLog.error({ err, agentName }, 'Avatar delete failed')
  ...
  router.get('/:name/avatar', (req, res) => {
    ...
    res.sendFile(avatarPath, (err) => {
      if (err) {
        avatarLog.error({ err, agentName, avatarPath }, 'Avatar sendFile failed')
        if (!res.headersSent) {
