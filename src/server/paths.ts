...
/** The `.kovitoboard/logs/` directory (DEC-017) */
export function getLogsDir(fs: FileAccessLayer): string {
  return join(getKovitoboardDir(fs), 'logs')
}
...
/**
 * Create the `.kovitoboard/logs/` directory if it does not exist
 * (DEC-017). Must be called before initLogger() so pino-roll has a
 * directory to write to.
 */
export function ensureLogsDir(fs: FileAccessLayer): void {
