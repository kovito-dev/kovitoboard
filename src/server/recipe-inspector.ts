...
// --- Pure declarative detection (DEC-006 v2.0 § 6) ---
...
/**
 * Heuristic patterns that indicate the artifact code reaches outside
 * the declarative handler model — anything from a hand-rolled HTTP
 * client to a child process spawn. The regex set is intentionally
 * coarse: false positives only surface a warning dialog and do not
 * block installation. Refinement to AST-level analysis is tracked as
 * a v0.2.0+ backlog item.
 *
 * @see DEC-006 v2.0 § 6, recipe-system.md v2.0 § 15-1
 */
const NON_DECLARATIVE_PATTERN_DETECTORS: PatternDetector[] = [
  { name: 'express-router', regex: /import\s+(?:.*\bRouter\b.*\s+)?from\s+['"]express['"]/g },
  { name: 'direct-fetch', regex: /\bfetch\s*\(/g },
  { name: 'axios-import', regex: /from\s+['"]axios['"]/g },
  { name: 'child-process', regex: /from\s+['"]child_process['"]/g },
  { name: 'node-fs-direct', regex: /from\s+['"](?:node:)?fs['"]/g },
  { name: 'shell-exec', regex: /\b(?:exec|execSync|spawn|spawnSync)\s*\(/g },
  { name: 'process-env-write', regex: /process\.env\.[A-Z_]+\s*=/g },
]

/**
 * Heuristically detect non-declarative patterns in recipe artifacts.
 *
 * Result is independent of the existing `findings` array — the
 * inspection verdict (blocked/warning/caution/safe) keeps its
 * pre-existing semantics, while `pureDeclarative` is a new axis
 * surfaced to the install UI to drive the warning dialog.
 *
 * @returns `{ pureDeclarative: true, detectedNonDeclarativePatterns: [] }`
 *          when no patterns matched any artifact; otherwise
 *          `pureDeclarative: false` with the deduplicated list of
 *          matched pattern names.
 *
 * @see DEC-006 v2.0 § 6-1
 * @see docs/specs/v0.1.0-recipe-install-handover.md §3.5
 */
export function analyzePureDeclarative(
  artifacts: ArtifactWithContent[],
): { pureDeclarative: boolean; detectedNonDeclarativePatterns: string[] } {
  const matched = new Set<string>()

  for (const artifact of artifacts) {
    // Only scan code-bearing files; JSON / Markdown cannot exercise
    // the runtime patterns we are looking for.
    const ext = extname(artifact.path)
    if (ext === '.json' || ext === '.md') continue

    for (const detector of NON_DECLARATIVE_PATTERN_DETECTORS) {
      detector.regex.lastIndex = 0
      if (detector.regex.test(artifact.content)) {
        matched.add(detector.name)
      }
    }
  }

  return {
    pureDeclarative: matched.size === 0,
    detectedNonDeclarativePatterns: [...matched],
  }
}
...
export async function inspectRecipe(recipe: ParsedRecipe): Promise<InspectionResult> {
  ...
  const declarativeAnalysis = analyzePureDeclarative(recipe.artifacts)
  ...
  return {
    ...
    note: remoteResult.skipped
      ...
      : undefined,
    pureDeclarative: declarativeAnalysis.pureDeclarative,
    detectedNonDeclarativePatterns: declarativeAnalysis.detectedNonDeclarativePatterns,
  }
}
