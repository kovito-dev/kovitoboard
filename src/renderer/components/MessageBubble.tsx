 ...
 */
export function UserMessageText({ text, onFilePathClick }: { text: string; onFilePathClick?: (path: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-1 mt-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      >
        ...
        <span className="text-xs">{copied ? t('chat.message.action.copied') : t('chat.message.action.copy')}</span>
      </button>
    </div>
  )
}
...
export function MessageBubble({ event, agentConfig, userConfig, onFilePathClick, theme = 'dark' }: MessageBubbleProps) {
  ...
  if (event.type === 'tool_result') {
    ...
    // Localize Claude Code's English "tool rejection" sentinel so the
    // message reads naturally for the user's locale. The raw English
    // text from the JSONL stays accessible via copy-to-clipboard for
    // debug, but the rendered card uses the localized string.
    if (event.metadata.interrupted === 'tool-rejected') {
      return (
        <div className="flex justify-center my-2">
          <span className="text-xs text-[var(--warning-text)] bg-[var(--warning-bg)] border border-[var(--warning-border)] px-3 py-1 rounded-full">
            {t('chat.message.interrupt.toolRejected')}
          </span>
        </div>
      )
    }
  ...
  // Localize Claude Code's "[Request interrupted ...]" user message
  // sentinel. We render it as a centered system-style pill instead of
  // a regular user bubble so the timeline visually represents that
  // this is metadata about the turn ending, not a real user utterance.
  if (event.type === 'user' && event.metadata.interrupted === 'user-interrupt') {
    return (
      <div className="flex justify-center my-2">
        <span className="text-xs text-[var(--warning-text)] bg-[var(--warning-bg)] border border-[var(--warning-border)] px-3 py-1 rounded-full">
          {t('chat.message.interrupt.userInterrupt')}
        </span>
      </div>
    )
  }
      ...
      <div className={`flex flex-col max-w-[92%] sm:max-w-[80%] md:max-w-[75%] ${isUser ? 'items-end' : 'items-start'}`}>
        ...
        >
          {event.content.text && (
            isUser
              ? (() => {
                  // Pull KB-authored sections (kbcontext / a11y / app-create / etc.)
                  // out of the message and render them as collapsible chips.
                  // Whatever the user actually typed remains plain text.
                  const parsed = parseKbAuthoredSections(event.content.text)
                  return (
                    <>
                      {parsed.sections.length > 0 && (
                        <KbAuthoredSections sections={parsed.sections} />
                      )}
                      {parsed.userInput.length > 0 && (
                        <UserMessageText
                          text={parsed.userInput}
