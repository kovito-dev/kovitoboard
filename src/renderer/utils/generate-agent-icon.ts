...
/**
 * Coerce an arbitrary input string to a 7-char `#rrggbb` literal,
 * falling back to a theme-appropriate neutral grey when the input is
 * undefined, empty, or not a valid 6-digit hex. `darkenColor` and
 * `withOpacity` both index into the input via fixed-position
 * `slice(...)` calls and would emit `NaN` -> rgb(NaN,NaN,NaN) (which
 * the browser interprets as black) for any malformed value, so we
 * normalise upstream.
 */
function normaliseThemeColor(input: string | undefined, theme: 'dark' | 'light'): string {
...
export function generateAgentIconSvg({ agentId, themeColor, theme = 'dark' }: AgentIconParams): string {
  ...
  const baseColor = normaliseThemeColor(themeColor, theme)
  const effectiveColor = isLight ? darkenColor(baseColor, 0.8) : baseColor
  const subColor = withOpacity(effectiveColor.startsWith('#') ? effectiveColor : baseColor, isLight ? 0.15 : 0.3)
